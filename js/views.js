...
(function () {
  ...
  function map() {
    const rows = T.places.map((p) => `
      <a class="place" href="https://www.google.com/maps/search/?api=1&query=${p.lat},${p.lng}" target="_blank" rel="noopener">
        <div><div style="font-weight:600">${esc(p.name)}</div><div class="caption ghost">Day ${p.day}</div></div>
        <div style="text-align:right"><div class="place__coord">${p.lat.toFixed(3)}, ${p.lng.toFixed(3)}</div><div class="caption" style="color:var(--accent)">Open ↗</div></div>
      </a>`).join("");
